...
3. リリースビルドを実行し、エラーがないことを確認する。
   ...
   npm run release
   ```
4. `npm run release` で `/home/takashi/server/www/calendar` まで自動反映されることを確認する。
...
3. 同期時に予定データに加えてクイックアクションの色設定（`早/中/遅/休`）も同期する。
4. 未同期の年を表示している間は、カレンダー上部に同期を促すメッセージを表示する。
